...
**There's no live browser UI right now — a real, verified gap, not an
oversight in this list.** `procurement-core`'s Fiori Elements UI
(`/$fiori-preview/...`, the one [Testing and navigating it](#testing-and-navigating-it)
walks through **locally**) 404s on the deployed instance — confirmed
directly. `$fiori-preview` is generated by `cds-dk` at dev/watch time;
the production build this app deploys with (`cds-deploy`, no dev-time
tooling) doesn't include it. Testing the live deployment means calling
the endpoints above directly (curl/Postman), not clicking through a UI.

**`procurement-core`'s protected OData endpoints (`/procurement/*`) also
can't be exercised live yet** — confirmed by testing directly: neither
an unauthenticated call, the local mocked-auth credentials (`bob:x`,
etc. — those only work with CAP's local dev profile, not deployed), nor
a call routed through `api-gateway` with a real API key gets past the
real, deployed XSUAA instance's `401`. This is the exact gap
`docs/operations/networking-and-request-flow.md` §3 already documents
("hasn't been exercised end-to-end by a real OAuth2 client yet") — no
real OAuth2 client/token exists to test with yet. `/health` above is
the one `procurement-core` route that doesn't need one.
...
or 404s where this table says it should 200, the app is almost
certainly just stopped, not gone — see
