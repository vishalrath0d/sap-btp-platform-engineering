...
> **Status: deployed and live on a real SAP BTP trial subaccount — both runtimes.** All five services below run locally with zero BTP account needed (see [Quick start](#quick-start)) — and are *also* actually deployed and reachable right now, four on Cloud Foundry and one (`spend-anomaly-detector`) on a real Kyma cluster, real infra provisioned by `infra/terraform`, real bugs found and fixed getting there on both runtimes. See [Live on BTP](#live-on-btp) for the full list of real URLs.
>
...
**Bugs found and fixed getting `spend-anomaly-detector` real on Kyma** — SAP approved the trial Kyma request; 5 more real, live-only bugs surfaced deploying against the actual cluster for the first time (full detail in `docs/next/next.md`'s session 12 entry): a Terraform adopt-lookup that couldn't see this trial's Kyma cluster at all (provisioned through SAP's own broker, not the self-service API Terraform queries — fixed the same way as HANA Cloud/XSUAA below, gated to `count=0`, managed outside Terraform instead); a CI auth model mismatch (the kubeconfig's browser-based OIDC login has no path in a headless GitHub Actions runner — fixed with a dedicated, narrowly-scoped `ServiceAccount` + static bearer token instead of trying to reuse a human session); an invalid APIRule v2 wildcard path (`/anomalies/*` isn't valid syntax — needs `{*}`); a missing Istio sidecar (the namespace needed `istio-injection=enabled`, and pods needed a restart to pick it up); and an XSUAA issuer mismatch (Istio's JWT filter needs the *exact* `iss` string, `<url>/oauth/token`, not the bare URL the service binding exposes). All fixed and codified into `kyma-deploy.yml`, not one-off manual patches. Verified end to end: `/health` (no auth) → real `200`; `/anomalies` with no token → real `403`; `/anomalies`, `/alerts`, `/admin/flags` with a real XSUAA token → real `200`s with real JSON.
