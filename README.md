...
**The live browser UI and a real OAuth2 login now both work** — closed
...
# real password-grant against the deployed XSUAA instance
TOKEN=$(curl -s "https://4cbf0c12trial.authentication.us10.hana.ondemand.com/oauth/token" \
  -u "<clientid>:<clientsecret>" \
  --data-urlencode "grant_type=password" \
  --data-urlencode "username=<your BTP username>" \
  --data-urlencode "password=<your BTP password>" | jq -r '.access_token')

curl -H "Authorization: Bearer $TOKEN" "$CORE/\$fiori-preview/ProcurementService/PurchaseRequisitions"
# -> real 200, full UI5-bootstrapped HTML
curl -H "Authorization: Bearer $TOKEN" "$CORE/procurement/PurchaseRequisitions"
# -> real seeded data: PR-00001 (CONVERTED), PR-00002 (SUBMITTED), ...
```

`procurement-core`'s Fiori Elements UI (`/$fiori-preview/...`) originally
404'd on the deployed instance — `$fiori-preview` is generated by
`cds-dk` at dev/watch time, and the production build (`cds-deploy`, no
dev-time tooling) doesn't include it by default. Fixed by explicitly
re-enabling it (`package.json`'s `cds.fiori.preview`/`routes: true` —
real CAP config, not a hack; see `services/procurement-core/README.md`).
`clientid`/`clientsecret` for the token request above come from
`procurement-core-xsuaa`'s service binding (`cf create-service-key
procurement-core-xsuaa <name>` then `cf service-key ... <name>`); `<your
BTP username/password>` is the same SAP Universal ID used for the
cockpit — real password-grant OAuth2, works because
`infra/terraform/modules/role-collections` now assigns that user to all
3 role collections (`btp_subaccount_role_collection_assignment`, applied
this session — see `infra/terraform/README.md`). Get a decoded token and
you'll see real scopes: `procurement-core!t700023.Requester`,
`.Approver`, `.IntegrationAdmin`.

**One real bug found while proving this end to end, not yet fixed**:
`POST /procurement/PurchaseRequisitions` (creating a *new* requisition)
500s on the deployed HANA-backed instance — `duplicate column name:
STATUS` in the generated INSERT. Root cause, confirmed from the app's
own logs: `srv/service-ui.cds`'s `status as statusText : String`
(a `@Common.Text` annotation helper for the Fiori UI) gets included in
HANA's generated INSERT and collides with the real `status` column —
SQLite (local dev) never hit this. Reads, `submit`, and `approve` on
*existing* seeded requisitions are unaffected — this only blocks
creating brand-new ones on the deployed instance. Flagged, not fixed
this session (see `services/procurement-core/README.md`'s "Known
limitations").
