...
- ~~XSUAA scopes aren't exercised by a real external client~~ —
  **resolved this session.** A real password-grant OAuth2 token
  (`grant_type=password`, the deployed XSUAA client's own
  `clientid`/`clientsecret` from `procurement-core-xsuaa`'s service
  binding, plus the real BTP user's own credentials) now genuinely
  works end to end: `Requester`/`Approver`/`IntegrationAdmin` scopes on
  the token, `GET /procurement/PurchaseRequisitions` returns real
  seeded data. What made this work: `infra/terraform/modules/role-
  collections` now assigns the real applying user
  (`btp_subaccount_role_collection_assignment`) to all 3 role
  collections — without it, no real user had the scopes the CDS model's
  `@requires` annotations check. See the root README's "Live on BTP"
  section for the exact runnable command.
  **One thing this surfaced that client_credentials grant cannot do**:
  tested first, client_credentials authenticates fine (its own real,
  valid token) but every call to `/procurement/*` 500s — CAP's
  authorization layer expects a genuine *user* identity (`req.user`),
  which a client-only (machine) token doesn't carry. Password grant
  (or, for a real production client, `authorization_code` via a proper
  login flow/approuter) is the correct grant type here, not
  `client_credentials`.
- ~~No live browser UI on the deployed instance~~ — **resolved this
  session.** `procurement-core`'s Fiori Elements preview
  (`/$fiori-preview/...`) originally 404'd when deployed — it's
  generated by `cds-dk` at dev/watch time, and the production build
  (`cds-deploy`, no dev-time tooling bundled) excludes it by default.
  Fixed by explicitly re-enabling `cds.fiori.preview`/`routes` in
  `package.json` (a real CAP config toggle, not a hack — see
  `services/procurement-core/README.md`). Confirmed live: `GET
