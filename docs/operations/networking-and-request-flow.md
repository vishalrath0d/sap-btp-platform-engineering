...
- **XSUAA scopes aren't exercised by a real external client.** The role
  ...
  tested via CAP's mocked auth instead. A genuine end-to-end token test
  (e.g. via `cf ssh` + `curl` with a real client-credentials grant) is a
  reasonable next verification step, not yet done.
