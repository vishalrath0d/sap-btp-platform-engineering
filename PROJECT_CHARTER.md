...
1. Add `abap`/`integration-suite` trial entitlements to `infra/terraform`'s `entitlements` module — **done and confirmed**: the first guessed names (`abap`/`trial`, `integration-suite`/`trial`) failed a real live `terraform apply` ("not entitled to this service plan"), but `btp list accounts/entitlement --subaccount <id>` (real command output) showed both are actually already granted, under different real names — `abap-trial`/`shared`, `integrationsuite-trial`/`trial` — confirmed again against the cockpit's Entitlements → Add Service Plans catalog. `main.tf` now uses the real values; see `infra/terraform/README.md` for the full live-apply story, including two more real bugs (`cloudfoundry`/`hana-cloud-trial` were *also* wrong names for already-granted entitlements, and the CF/Kyma adopt-lookup didn't check instance health state, so it could have silently adopted a failed Kyma instance).
...
| `spend-anomaly-detector` | Kyma-native event-driven microservice, subscribes to PO-created events via Event Mesh — **deployed for real on the trial Kyma cluster** (BTP Operator + APIRule v2, `k8s/`), SAP approved the trial request 2026-08-27, verified end-to-end including real gateway-level JWT auth (see `docs/next/next.md` session 12). The earlier temporary Cloud Foundry detour (`manifest.yml`) is gone — this is its permanent home now, though the trial cluster itself auto-expires ~2026-09-08, a real deadline tracked in `docs/next/next.md`'s "Next steps" |
