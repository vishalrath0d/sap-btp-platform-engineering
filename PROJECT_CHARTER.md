...
| `spend-anomaly-detector` | Kyma-native event-driven microservice, subscribes to PO-created events via Event Mesh — real Kyma deployment (BTP Operator + APIRule v2) written and ready, see `k8s/`. **Deployed to Cloud Foundry temporarily** (`manifest.yml`, wired into `cf-deploy.yml`) while a trial Kyma cluster request is pending SAP's approval (see `infra/terraform`'s `kyma_enabled` flag and `docs/next/next.md`) — the `k8s/` manifests and Kyma deploy workflows are untouched, ready to become the real path again the moment approval lands |
